...
4. **SecurityConfig** — HeaderAuthenticationFilter 사용 (item-service 참고)

5. **WebMvcConfig** — CurrentUserIdArgumentResolver 등록 (item-service 참고)

6. **api-gateway/application.yml에 라우트 추가**:
   ```yaml
   - id: order-service
...
flowchart TB
    ...
    subgraph ItemSvc["item-service :8087 (예시)"]
    ...
    %% 로그인 흐름
    Client -->|"① POST /api/auth/login\n(email + password)"| UserSvc
    US -->|"② JWT 발급\n(accessToken + refreshToken)"| Client
    US <-->|"③ Refresh Token 저장/조회"| Redis

    %% 일반 API 흐름
    Client -->|"④ API 요청\nAuthorization: Bearer {JWT}"| Gateway
    GW -->|"⑤ JWT 검증 후\nX-User-Id / X-User-Role 헤더 추가"| ItemSvc
