...
curl -X POST \
     -H "Content-Type: application/json" \
     -d '{"email":"user1@bari.com","password":"password123"}' \
...
### 새 서비스 추가 (예: order-service)

1. **디렉토리 생성**:
   ```
   services/order-service/
   ├── build.gradle
   └── src/main/java/com/bari/order/
   ```
...
3. **build.gradle 작성** (item-service를 참고):
   ```groovy
   plugins {
...
로컬 개발 시 아래 계정으로 바로 테스트할 수 있습니다 (비밀번호: `password123`):
